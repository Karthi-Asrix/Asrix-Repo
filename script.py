...
@app.post("/V4.1", summary="Testing RAG V4.1")
async def rag2(request_data: RAGLlm):
    """
        Naive RAG with added functionality:
        - Data persistence in local file ./store/
    ...
    result = retriever_pdf.get_relevant_documents(request_data.prompt)
...
async def rag4_1(request_data: RAGLlm2, db_fastapi: Session = Depends(get_db)):
    """
        Naive RAG with added functionality:
        - Vector database saved based on character ID and documents for each
          character ID can be tracked inside character/document_logs.txt
    ...
    result = retriever.get_relevant_documents(request_data.prompt)
    ...
    db_data.context = str(result)
    db_data.prompt = request_data.prompt
    db_data.response = response

    db_fastapi.add(db_data)
    db_fastapi.commit()

    return response

@app.get("/")
def read_api(db_fastapi: Session = Depends(get_db)):
    return db_fastapi.query(models.FastApiData).all()
...
@app.delete("/{id}")
def delete(id: int, db_fastapi: Session = Depends(get_db)):

    db_data = db_fastapi.query(models.FastApiData).filter(models.FastApiData.id == id).first()
    ...
    db_fastapi.query(models.FastApiData).filter(models.FastApiData.id == id).delete()
